...
-- AutoSession
vim.cmd("au VimEnter * nested if (len(v:argv) == 2) | silent! source Session.vim | endif")
vim.cmd("au VimLeave * if (len(v:argv) == 2) | mksession! | endif")
...
vim.cmd("au Filetype qf nnoremap <buffer> q :call setqflist([])<CR>:q<CR>")
...
vim.keymap.set("n", "]h", function() require("gitsigns").next_hunk({ wrap = false }) end)
vim.keymap.set("n", "[h", function() require("gitsigns").prev_hunk({ wrap = false }) end)
...
vim.keymap.set("n", "<leader>t", require("fzf-lua").buffers)
vim.keymap.set("n", "<leader>n", require("fzf-lua").tabs)
...
vim.keymap.set("n", "<localleader>o", ":let @+ = expand('%')<CR>")
...
vim.keymap.set("n", "<localleader>d", require("fzf-lua").diagnostics_workspace)
